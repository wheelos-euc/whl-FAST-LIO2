...
void LI_Init::set_vehicle_prior(const M3D &R_LI_prior, const V3D &T_LI_prior,
                                double roll_pitch_sigma_deg,
                                double yaw_sigma_deg,
                                double translation_sigma_m) {
    vehicle_prior_enabled = true;
    Rot_Lidar_wrt_IMU_prior = R_LI_prior;
    Trans_Lidar_wrt_IMU_prior = T_LI_prior;
    roll_pitch_prior_sigma_deg = roll_pitch_sigma_deg;
    yaw_prior_sigma_deg = yaw_sigma_deg;
    translation_prior_sigma_m = translation_sigma_m;
    Rot_Lidar_wrt_IMU = R_LI_prior;
    Trans_Lidar_wrt_IMU = T_LI_prior;
}
...
void LI_Init::solve_Rotation_only() {
    double R_LI_quat[4];
    Eigen::Quaterniond init_quat(vehicle_prior_enabled ? Rot_Lidar_wrt_IMU_prior : Eye3d);
    R_LI_quat[0] = init_quat.w();
    R_LI_quat[1] = init_quat.x();
    R_LI_quat[2] = init_quat.y();
    R_LI_quat[3] = init_quat.z();
    ...
    if (vehicle_prior_enabled) {
        problem_rot.AddResidualBlock(
                Rotation_Prior_Cost::Create(Rot_Lidar_wrt_IMU_prior,
...
void LI_Init::solve_trans_biasacc_grav() {
    ...
    V3D trans_il_init = vehicle_prior_enabled
                            ? -Rot_Lidar_wrt_IMU.transpose() * Trans_Lidar_wrt_IMU_prior
                            : Zero3d;
    Trans_IL[0] = trans_il_init.x();
    Trans_IL[1] = trans_il_init.y();
    Trans_IL[2] = trans_il_init.z();
    ...
    if (vehicle_prior_enabled) {
        problem_acc.AddResidualBlock(
                Translation_Prior_Cost::Create(trans_il_init,
...
bool LI_Init::data_sufficiency_assess(MatrixXd &Jacobian_rot, int &frame_num, V3D &lidar_omg, int &orig_odom_freq,
    ...
    if (frame_num % orig_odom_freq * cut_frame_num == 0) {
        ...
        last_rot_progress = Rot_percent_scaled;
...
void LI_Init::plot_result() {
    // Apollo migration keeps visualization outside the calibration core.
}
