...
struct Rotation_Prior_Cost {
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW

    Rotation_Prior_Cost(const M3D &prior_rot_, double roll_pitch_sigma_deg,
                        double yaw_sigma_deg)
            : prior_q(prior_rot_) {
        const double rp_sigma_rad =
                std::max(1e-3, roll_pitch_sigma_deg * M_PI / 180.0);
        const double yaw_sigma_rad =
                std::max(1e-3, yaw_sigma_deg * M_PI / 180.0);
        inv_roll_pitch_sigma = 1.0 / rp_sigma_rad;
        inv_yaw_sigma = 1.0 / yaw_sigma_rad;
        prior_q.normalize();
    }
    ...
    bool operator()(const T *q, T *residual) const {
        Eigen::Quaternion<T> q_LI{q[0], q[1], q[2], q[3]};
        Eigen::Quaternion<T> prior_q_T(T(prior_q.w()), T(prior_q.x()),
                                       T(prior_q.y()), T(prior_q.z()));
        Eigen::Quaternion<T> delta_q = prior_q_T.conjugate() * q_LI;
        residual[0] = T(2.0) * delta_q.x() * T(inv_roll_pitch_sigma);
        residual[1] = T(2.0) * delta_q.y() * T(inv_roll_pitch_sigma);
        residual[2] = T(2.0) * delta_q.z() * T(inv_yaw_sigma);
        return true;
    }

    static ceres::CostFunction *Create(const M3D &prior_rot_,
                                       double roll_pitch_sigma_deg,
                                       double yaw_sigma_deg) {
        return (new ceres::AutoDiffCostFunction<Rotation_Prior_Cost, 3, 4>(
                new Rotation_Prior_Cost(prior_rot_, roll_pitch_sigma_deg,
                                        yaw_sigma_deg)));
    }

    Eigen::Quaterniond prior_q;
    double inv_roll_pitch_sigma = 1.0;
    double inv_yaw_sigma = 1.0;
};

struct Translation_Prior_Cost {
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW

    Translation_Prior_Cost(const V3D &prior_trans_il_, double sigma_m)
            : prior_trans_il(prior_trans_il_) {
        inv_sigma = 1.0 / std::max(1e-3, sigma_m);
    }

    template<typename T>
    bool operator()(const T *trans_il, T *residual) const {
        residual[0] = (trans_il[0] - T(prior_trans_il.x())) * T(inv_sigma);
        residual[1] = (trans_il[1] - T(prior_trans_il.y())) * T(inv_sigma);
        residual[2] = (trans_il[2] - T(prior_trans_il.z())) * T(inv_sigma);
        return true;
    }

    static ceres::CostFunction *Create(const V3D &prior_trans_il_,
                                       double sigma_m) {
        return (new ceres::AutoDiffCostFunction<Translation_Prior_Cost, 3, 3>(
                new Translation_Prior_Cost(prior_trans_il_, sigma_m)));
    }
...
public:
    ...
    void set_vehicle_prior(const M3D &R_LI_prior, const V3D &T_LI_prior,
                           double roll_pitch_sigma_deg, double yaw_sigma_deg,
    ...
    bool data_sufficiency_assess(MatrixXd &Jacobian_rot, int &frame_num, V3D &lidar_omg, int &orig_odom_freq,
                                 int &cut_frame_num);
    V3D get_last_rot_progress() const { return last_rot_progress; }
...
private:
    ...
    bool vehicle_prior_enabled = false;
    M3D Rot_Lidar_wrt_IMU_prior = Eye3d;
    V3D Trans_Lidar_wrt_IMU_prior = Zero3d;
    double roll_pitch_prior_sigma_deg = 5.0;
    double yaw_prior_sigma_deg = 10.0;
    double translation_prior_sigma_m = 0.5;
    V3D last_rot_progress = Zero3d;
